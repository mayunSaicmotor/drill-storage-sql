...
/*			HttpGroupScan groupScan = (HttpGroupScan) scan.getGroupScan();
	...
	public static final StoragePluginOptimizerRule COMMON_SORT_ON_SCAN = new PushDownOrderByToScan(
		...
		public void onMatch(RelOptRuleCall call) {
			...
			final SortPrel sort = (SortPrel) call.rel(0);
			logger.debug("COMMON_SORT_ON_SCAN onMatch");
	...
	public static final StoragePluginOptimizerRule COMMON_DRILL_SORT_REL = new PushDownOrderByToScan(
			RelOptHelper.any(DrillSortRel.class), "PushDownOrderByToScan:Drill_Sort_Rel") {
		@Override
		public void onMatch(RelOptRuleCall call) {
			...
			//TODO
			//List<String> orderFields = NodeProcessUtil.getOrderBycols(sort);
			Map<Integer, OrderByColumn> orderFieldsMap = NodeProcessUtil.getOrderByColsMap(sort.getCollationList(), sort.getRowType().getFieldList());
			if(orderFieldsMap == null){
				return;
			}			
			setOrderFieldsMap(groupScan, orderFieldsMap);
	...
	//it can get the  OrderByCols but sometimes it can't be push down(call.transformTo(scan))
	// TODO for multi-tables query, need to confirm the result
	public static final StoragePluginOptimizerRule TOPN_ON_PROJECT_HASHTORANDOMEXCHANGE = new PushDownOrderByToScan(
			RelOptHelper.some(TopNPrel.class, RelOptHelper.some(ProjectPrel.class, RelOptHelper.any(HashToRandomExchangePrel.class))), "PushDownOrderByToScan:Common_TopN_On_Project_HashToRandomExchange") {
		@Override
		public void onMatch(RelOptRuleCall call) {
		...
		@Override
		public boolean matches(RelOptRuleCall call) {
			final Prel sort = (Prel) call.rel(0);

			if (sort instanceof TopNPrel) {
	...
	public static final StoragePluginOptimizerRule TOPN_ON_PROJECT_HASHAGG_HASHTORANDOMEXCHANGE = new PushDownOrderByToScan(
			RelOptHelper.some(TopNPrel.class, RelOptHelper.some(HashAggPrel.class, RelOptHelper.some(ProjectPrel.class, RelOptHelper.any(HashToRandomExchangePrel.class)))), "PushDownOrderByToScan:Common_TopN_On_Project_HashAgg_HashToRandomExchange") {
		@Override
		public void onMatch(RelOptRuleCall call) {
	...
	public void setUpOrderByFields(RelOptRuleCall call) {
		final TopNPrel sort = (TopNPrel) call.rel(0);
		logger.debug("Http onMatch");
...
/*		if (groupScan.isOrderByPushedDown()) {
		...
		findAndSetOrderByFields(sort, groupScan);
	...
	public static void findAndSetOrderByFields(final Prel sort, HttpGroupScan groupScan) {
		Map<Integer, OrderByColumn>  orderFieldsMap = NodeProcessUtil.getOrderByColsMap(sort);
		if(orderFieldsMap == null){
			return;
		}			
